...
import { BuildUrl, ErrorOnBadStatus, ArrayDeepEquals, bidsApiPath, jobsApiPath } from './CommonApi';
import BidModel from '../models/BidModel';

export default class BidApi {
  // Returns an observable, which generates an array of all your bids
  static MyBids() {
    return axios.get(BuildUrl(bidsApiPath)).pipe(ErrorOnBadStatus);
  }
  ...
  static GetBid(id) {
    return axios.get(BuildUrl(bidsApiPath, id)).pipe(ErrorOnBadStatus);
  }
  ...
  static GetBidsForJob(jobid) {
    return axios.get(BuildUrl(jobsApiPath, jobid, 'bid')).pipe(ErrorOnBadStatus);
  }
  ...
  static CreateBid(bidModel, jobId) {
    return axios.post(BuildUrl(jobsApiPath, jobId, bidsApiPath), bidModel).pipe(ErrorOnBadStatus);
  }
  ...
  static UpdateBid(id, bidModel) {
    return axios.put(BuildUrl(bidsApiPath, id), { params: bidModel }).pipe(ErrorOnBadStatus);
  }
